...
<form action="{{ route('user.store') }}" method="POST" class="space-y-4">
    ...
    <div>
        ...
        <input type="text" name="username" class="input input-bordered w-full" value="{{ old('username') }}" required>
        @error('username') <div class="text-red-500 text-sm">{{ $message }}</div> @enderror
    </div>
    <div>
        <label class="block mb-1">Password</label>
        <input type="text" name="password" class="input input-bordered w-full" value="{{ old('password') }}" required>
